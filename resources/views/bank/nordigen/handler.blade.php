...
<script>
    ...
    const config = {
        ...
        text: "{{ ($account ?? false) && !$account->isPaid() ? 'BizLab IT Solutions' : (isset($company) && !is_null($company) ? $company->name : 'BizLab IT Solutions') }} {{ ctrans('texts.nordigen_handler_subtitle', [], $lang ?? 'en') }}",
        // Logo URL that will be shown below the modal form.
        logoUrl: "{{ ($account ?? false) && !$account->isPaid() ? asset('images/logo.png') : (isset($company) && !is_null($company) ? $company->present()->logo() : asset('images/logo.png')) }}",
